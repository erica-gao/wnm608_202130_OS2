

    <div class="grid gap product-list">
      <div class="col-xs-12 col-md-4">
         <a href="product_item.php">
            <div class="three_item_image_wrap" style="background-image: url(images/capsule.jpg);"></div>
         </a>

         <div class="product_name"><h4>Product Name</h4></div>
         <div class="product_name"><p style="text-align: center;">Lorem ipsum dolor sit amet consectetur adipisicing elit. Iure praesentium similique molestias tenetur perferendis alias.</p></div>
         <p class="button big" style="text-align: center;">Add to Cart</p>
      </div>

      <div class="col-xs-12 col-md-4">
         <a href="product_item.php">
            <div class="three_item_image_wrap" style="background-image: url(images/product2.jpg);"></div>
         </a>

         <div class="product_name"><h4>Product Name</h4></div>
         <div class="product_name"><p style="text-align: center;">Lorem ipsum dolor sit amet consectetur adipisicing elit. Iure praesentium similique molestias tenetur perferendis alias.</p></div>
         <p class="button big" style="text-align: center;">Add to Cart</p>
      </div>

      <div class="col-xs-12 col-md-4">
         <a href="product_item.php">
            <div class="three_item_image_wrap" style="background-image: url(images/product3.jpg);"></div>
         </a>

         <div class="product_name"><h4>Product Name</h4></div>
         <div class="product_name"><p style="text-align: center;">Lorem ipsum dolor sit amet consectetur adipisicing elit. Iure praesentium similique molestias tenetur perferendis alias.</p></div>
         <p class="button big" style="text-align: center;">Add to Cart</p>
      </div>
   </div>

  

		